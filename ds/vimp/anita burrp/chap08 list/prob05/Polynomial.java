...
public class Polynomial {
    ...
    public Polynomial(double[] c) {  // Programming Problem 8.5
        Node p = head;
    ...
    public Polynomial plus(Term term) {
        ...
        while (p.next.term.getExp() > termExp)
            p = p.next;
        if (p.next.term.getExp()==termExp) p.next.term = p.next.term.plus(term);
    ...
    public Polynomial times(Polynomial polynomial) {
        if (this.equals(ZERO) || polynomial.equals(ZERO)) return ZERO;
        Polynomial result = ZERO;
        for (Node p = head.next; p != head; p = p.next)
